...
async function inviteEditor(data: { name: string; email: string }, currentUser?: AuthUser) {
  ...
  if (existingInvitation && new Date() < existingInvitation.ttl) {
    console.log(`⚠️ [Invite Editor] Pending invitation already exists for: ${email}`);
    throw new BadRequestError("An invitation has already been sent to this email. Please wait for it to expire or ask the editor to check their inbox.");
  }
  ...
  console.log(`✅ [Invite Editor] Verification token created: ${token.substring(0, 10)}...`);
  console.log(`⏰ [Invite Editor] Token expires at: ${expiresAt}`);

  // Send invitation email
  console.log(`📧 [Invite Editor] Attempting to send invitation email to: ${email}`);
  console.log(`🔗 [Invite Editor] Frontend URL: ${process.env.FRONTEND_URL || 'http://localhost:3000'}`);
  
  try {
    await sendEditorInvitationEmail(email, name, token);
    console.log(`✅ [Invite Editor] Invitation email sent successfully to: ${email}`);
  } catch (error) {
    console.error(`❌ [Invite Editor] Failed to send invitation email to: ${email}`);
