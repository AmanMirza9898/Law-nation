...
// 1. Separate the Form into its own component
function LoginForm() {
  const router = useRouter()
  const dispatch = useDispatch()
  const searchParams = useSearchParams();
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(data.message || "Invalid credentials", { toastId: "login-error" });
      }
    } catch (error) {
      console.warn("Login Error:", error)
      toast.error("Something went wrong. Please try again.", { toastId: "net-error" });
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="h-screen flex overflow-hidden">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center bg-gray-50 px-4 sm:px-6 lg:px-10 xl:px-12 py-6 overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          <Suspense fallback={<div className="text-center py-10 text-gray-500">Loading form...</div>}>
            <LoginForm />
          </Suspense>
        </div>
      </div>
